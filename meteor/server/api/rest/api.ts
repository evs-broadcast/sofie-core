...
class ServerRestAPI implements RestAPI {
	...
	async executeAdLib(
		...
		if (regularAdLibDoc) {
			...
			const rundownPlaylist = RundownPlaylists.findOne(rundownPlaylistId, {
				projection: { currentPartInstanceId: 1, activationId: 1 },
			})
	...
	async addStudio(
		...
		const newStudioId = Studios.insert(newStudio)
		...
		return ClientAPI.responseSuccess(unprotectString(newStudioId))
	}
...
sofieAPIRequest<{ playlistId: string }, { adLibId: string; actionType?: string; adLibOptions?: any }, object>(
	...
	async (serverAPI, connection, event, params, body) => {
		...
		logger.info(
			`koa PUT: executeAdLib ${rundownPlaylistId} ${adLibId} - triggerMode: ${triggerMode} - options: ${
